...
migrate:
	$(foreach ENV_NAME,$(ENV_LIST),\
		terraform -chdir=iac/environments/$(ENV_NAME) init -migrate-state -backend-config "bucket=$(PROJECT_PREFIX)-tf-states" -backend-config "prefix=terraform/$(PROJECT_PREFIX)$(ENV_NAME)/state";)
	@terraform -chdir=iac/environments/repo init -migrate-state -backend-config "bucket=$(PROJECT_PREFIX)-tf-states" -backend-config "prefix=terraform/$(PROJECT_PREFIX)repo/state"
...
init:
	...
	@terraform -chdir=iac/environments/repo init -backend-config "bucket=$(PROJECT_PREFIX)-tf-states" -backend-config "prefix=terraform/$(PROJECT_PREFIX)repo/state"
